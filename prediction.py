...
def main():
    ...
    for idx, batch in enumerate(tqdm(test_batches, desc="Processing batches")):
        ...
        for batch_idx, test_triple in enumerate(batch):
            ...
            selecting_prompt = SELECTING_PROMPT.format(fewshot_triples="\n".join(data_manager.triple_to_sentence(triple) for triple in fewshot_triples), test_triple=data_manager.triple_to_sentence(test_triple))
            # print(selecting_prompt)
            select_response = run_llm_chat([{"role": "user", "content": selecting_prompt}])
